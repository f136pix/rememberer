...
import {ICreateTask, ITask, ITaskReq, ITeam} from "src/types";

import {createTask, getTasksByUser, getTeams, updateTaskById, updateUserById} from "@/services/api/graphQl/graphQlApi.ts";
...
export const useGetTaskByUser = () => {
    return useMutation(
        {
            mutationFn: async (data: ITaskReq): Promise<any> => {
                try {
...
export const useCreateTask = () => {
    return useMutation(
        {
            mutationFn: async (data : ICreateTask): Promise<any> => {
                try {
